...
func ListaUsuarios(w http.ResponseWriter, r *http.Request) {
	...
	//vamos a capturar los parametros quevienen de la URL
	//en type indico si quiero los usuarios que yo sigo o los que no sigo
	typeUser := r.URL.Query().Get("type")
	//en page la pagina de resultados que estoy mostrando
	page := r.URL.Query().Get("page")
	//en search tengo el termino de busqueda que puede venir o no venir
	...
	pag := int64(pageTemp)
	//llamo a la funcion de bd con el ID del usuario que esta loguead y lo que vino
	...
	//si no encontro registro el status va a venir en true y result en vacio
	//viene todo bien
	...
	//le devolvemo la lista de resultados al navegador
